...
console.log('-- wiring pins (validate.js lane) --');
{
  ...
  assert('past-season prune runs over the five save boxes (+ casual solo rows, client knife 3.7a)', /for \(const \[sbxId, sbxLabel\] of \[\[saveBoxId, 'save box'\], \[compFam\.DUO\.saveBoxId, 'duo save box'\], \[compFam\.TRIO\.saveBoxId, 'trio save box'\], \[compFam\.QUAD\.saveBoxId, 'quad save box'\], \[casualSaveBoxId, 'casual save box'\]\]\)/.test(src));
